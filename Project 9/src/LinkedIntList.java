...
public class LinkedIntList
{
	...
	//This method stretches the linked int list by multiplying the number of
	//occurrences of each value in the list by the factor, n. If n is less than
	//1, it clears the list.
	public void stretch(int n){
		...
		for(int i=0; i<initialSize;i++){
			...
			for(int j=0; j<(n-1); j++){
				current.next = new ListNode(number, current.next);
				current = current.next;
			}
